...
int relTotal = 8;
  ...
  double TCValue_1, TCValue_2, TCValue_3, TCValue_4, TCValue_5, TCValue_6, TCValue_7, TCValue_8;
...
typedef struct {
  ...
  float x, vx, ax, z, mass, stripx, rpm, stripv, stripa; //enter vz and az after
  //Later: put in variables for storing data on all orientations
  float HP, AP, BP, CP, DP, PB1P, PB2P, PB3P, EB1tankP, EB1P, EB2tankP, EB2P, LC1P, LC2P; //Air system pressure values
  float HT, AT, BT, CT, DT, PB1T, PB2T, PB3T, EB1tankT, EB1T, EB2tankT, EB2T, LC1T, LC2T; //Air system temperature values
...
//FUNCTIONS
void printCC(double a[][COUNT]);//Print out the current Conditions and Counters matrix
int loadCC(double a[][COUNT]);//Populate the C&C matrix by reading from the CC text file
int genPhys(pod * p, float t1, float t2); //Calculate net force and determine velocity and position of pod
int readAirSuppSensors(pod * p);//Read signals from air supply pressure transducers and thermocouples
int readTempSensors(pod * p);//Read signals from the temperature sensors
int CCdecision(pod * p);//Make decisions based on whether collected data causes conditions to be satisfied
int transmit(pod * p);
...
int STANDBY() {
  ...
  while (kill == 0) {
    ...
    CP = 37.28 * CP - 3.2553;
    DP = 37.28 * DP - 3.2553;
    Serial.println(String(HP) + " " + String(AP) + " " + String(BP) + " " + String(CP) + " " + String(DP));
...
int RUN() {
  ...
  //Create timers for use by genPhys function. These measure time before and after all functions finish
  float t0 = float(millis()) / 1000, t1 = 0, t2 = 0; //to do: change t1 t2 to floats
  float timetrack = 0, tinc = 0.5; //tinc determines interval between printing pod motion properties
  float simTime = SIM_TIME;
  int ABORT = 0;
  skateSolActivate();
  while (poddy.x <= TRACK_LENGTH && kill == 0) { //while(simTime-t2 >= 0 && poddy.x <= TRACK_LENGTH){
...
int genPhys(pod * p, float t1, float t2) {
  ...
  float mu = 0.0005 + p->PBOn * (0.3395); //friction coefficient. Changes depending on whether primary brakes on
  float g = 9.8;
  int pusherStopDistance = ACC_LENGTH;//Maximum distance to which pusher can push the pod
  float pushAcc = PUSH_ACC;//Expected magnitude of pod acceleration resulting from pusher
  ...
  //Determine which direction the friction and drag force should be acting in. This check results in an
  //oscillation between positive and negative forces, likely due to the discrete time of the simulation.
  //ie, as the time interval goes to zero, the oscillation amplitude should go to zero and the velocity
  ...
  fric = sign * (p->skatesOff * p->mass * g * mu + p->EBOn * p->mass * 10.4); //To do for later: Fn=mg-skateForce;
...
int CCdecision(pod * p) {
  float xremain = TRACK_LENGTH - p->x;//Distance from pod to the end of the track
  float podvx = p->vx;
  float podax = p->ax;
  float mindecel = -1 * podvx * podvx / (2 * xremain); //minimum deceleration required to reach zero velocity by end of track
  float predPBfric = -9.8 * 0.34; //Predicted friction due to deceleration by primary brakes
  float predEBfric = -10.4;//Predicted friction due to deceleration by emergency brakes
  //Acceleration Phase
  if (p->pusherOn == 0) {
    //Coast phase
    if (podax >= mindecel) {
    ...
    else if (podax <= 1.07 * mindecel && p->PBOn == 1) { //If pod is decelerating too quickly (7% over), turn off the primary brakes
      p->PBOn = 0;
    }
    else {
      //Continue
...
int transmit(pod * p) {
  String data = String(p -> x) + " " + String(p -> vx) + " " + String(p -> ax) + " " + String(p->HP) + " " + String(p->AP) + " " + String(p->BP) + " " + String(p->CP) + " " + String(p->DP)+" " + String(p->HT)+" " + String(p->AT)+" " + String(p->BT)+" " + String(p->CT)+" " + String(p->DT);
  Serial.println(data);
  Serial.flush();
  return 0;
}
